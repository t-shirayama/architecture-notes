...
```mermaid
flowchart LR
    A[利用者・呼び出し元] --> B[Redux Architecture]
    B --> C[主要コンポーネント]
    C --> D[データ・外部システム]
```
...
| 比較対象 | 違い |
| --- | --- |
| Flux | Fluxは関連する問題領域で使われる。Redux Architectureは「単一のStore、Action、Reducerによってアプリケーション状態を予測可能に管理する構成です。」点を主に扱うため、導入目的と責務境界を分けて判断する |
| MVVM | MVVMは関連する問題領域で使われる。Redux Architectureは「単一のStore、Action、Reducerによってアプリケーション状態を予測可能に管理する構成です。」点を主に扱うため、導入目的と責務境界を分けて判断する |
| Feature-Sliced Design | Feature-Sliced Designは関連する問題領域で使われる。Redux Architectureは「単一のStore、Action、Reducerによってアプリケーション状態を予測可能に管理する構成です。」点を主に扱うため、導入目的と責務境界を分けて判断する |
