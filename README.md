...
| アーキテクチャ | 概要 | 配置先 |
| --- | --- | --- |
| レイヤードアーキテクチャ | Presentation / Application / Domain / Infrastructure のように層で分ける基本形 | `docs/レイヤー・責務分離系/` |
| クリーンアーキテクチャ | 依存関係を外側から内側へ向け、ビジネスルールを中心に置く | `docs/レイヤー・責務分離系/` |
| ヘキサゴナルアーキテクチャ | Ports & Adapters とも呼ばれ、ドメインを外部技術から分離する | `docs/レイヤー・責務分離系/` |
| オニオンアーキテクチャ | ドメインモデルを中心に置き、外側にアプリケーションやインフラを配置する | `docs/レイヤー・責務分離系/` |
| DDDアーキテクチャ | エンティティ、値オブジェクト、集約、リポジトリ、UseCaseなどで業務知識を表現する | `docs/レイヤー・責務分離系/` |
